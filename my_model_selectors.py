...
class SelectorBIC(ModelSelector):
    ...
    def select(self):
        ...
        best_score, best_model  = float("inf"), self.base_model(self.n_constant)

        for n_components in range(self.min_n_components, self.max_n_components + 1):
            try:
                model = self.base_model(n_components)
                logL = model.score(self.X, self.lengths)
                n_params = n_components**2 + 2*n_components*len((self.X[0])) - 1
...
class SelectorDIC(ModelSelector):
    ...
    def select(self):
        ...
        best_dic, best_model = float("-inf"), self.base_model(self.n_constant)
            ...
            try:
                ...
                for w in self.words:
                    if w != self.this_word:
                        ...
                        logL_rest.append(model.score(X, lengths))
                penalty = np.average(logL_rest)
                dic = logL - penalty
                if dic > best_dic:
...
class SelectorCV(ModelSelector):
    ...
    def select(self):
        ...
        if len(self.sequences) < 3:
            return best_model
        for n_components in range(self.min_n_components, self.max_n_components + 1):
            scores, splits = [], KFold(min(3, len(self.sequences))).split(self.sequences)
            ...
            if avg > best_score:
                best_score, best_model = avg, self.base_model(n_components)
